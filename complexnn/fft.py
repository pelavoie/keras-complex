...
def fft(z):
	...
	Zr     = tf.cond(pred=isOdd, true_fn=tf.concat([Zr, C*Zr[:,1:  ][:,::-1]], axis=1),
	                        false_fn=tf.concat([Zr, C*Zr[:,1:-1][:,::-1]], axis=1))
	Zi     = tf.cond(pred=isOdd, true_fn=tf.concat([Zi, C*Zi[:,1:  ][:,::-1]], axis=1),
	                        false_fn=tf.concat([Zi, C*Zi[:,1:-1][:,::-1]], axis=1))
	Zi     = (C*Zi)[:,:,::-1]  # Zi * i
	Z      = Zr+Zi
